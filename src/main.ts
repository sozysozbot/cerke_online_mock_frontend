...
import { GAME_STATE, fromAbsoluteCoord, toAbsoluteCoord, Season, Log2_Rate, initial_board_with_IA_down } from "./game_state";
import { animateOpponentTamNoStep, animateOpponentSrcDst, animateOpponentTamSteppingDuringFormer, animateOpponentFromHand, animateOpponentTamSteppingDuringLatter, animateOpponentSrcStepDstFinite, animateOpponentInfAfterStep, animateNode } from "./opponent_move";
import {
    toPath_,
    Coord,
    Piece,
...
import { calculateMovablePositions, canGetOccupiedByNonTam, canGetOccupiedBy } from "./calculate_movable";
...
export async function sendStuffTo<T, U>(api_name: string, log: string, message: T, validateInput: (response: any) => U): Promise<U> {
...
export function endSeason(base_score: number, is_first_move_my_move_in_the_next_season: boolean | null) {
    const score_display = document.getElementById("score_display")!;
    score_display.classList.add("nocover");
